...
class TestMafValidationError(unittest.TestCase):
    ...
    __errors = (
        MafValidationError(tpe=MafValidationErrorType.HEADER_LINE_EMPTY_KEY,
                           message="Error with line number"),
        MafValidationError(tpe=MafValidationErrorType.HEADER_LINE_MISSING_START_SYMBOL,
                           message="Error without line number", line_number=42)
    )

    def test_str(self):
        actual_strings = [str(error) for error in TestMafValidationError.__errors]
        expected_strings = ["Error with line number", "On line number 42: Error without line number"]
    ...
    def test_process_validation_errors_strict(self):
        ...
        with self.assertRaises(MafFormatException) as context:
            MafValidationError.process_validation_errors(validation_errors=TestMafValidationError.__errors,
                                                         validation_stringency=ValidationStringency.Strict,
                                                         logger=logger)
        self.assertIn("Error with line number", str(context.exception))
        self.assertTrue(context.exception.tpe, MafValidationErrorType.HEADER_LINE_EMPTY_KEY)
    ...
    def test_process_validation_errors_lenient(self):
        ...
        expected_lines = [MafValidationError.ignore_message(error) for error in TestMafValidationError.__errors]
        reader.close()
