...
class TestMafHeader(unittest.TestCase):
    ...
    def test_from_line_reader_ok(self):
        ...
        header = MafHeader.from_line_reader(
            line_reader=line_reader, validation_stringency=ValidationStringency.Silent
        )
        fh.close()
    ...
    def test_from_lines_supported_sort_order(self):
        for so in sort_order.SortOrder.all():
            lines = [
                ...
                TestMafHeader.__annotation_line,
                "%s%s %s"
                % (MafHeader.HeaderLineStartSymbol, MafHeader.SortOrderKey, so.name()),
            ]
            header = MafHeader.from_lines(
                lines=lines, validation_stringency=ValidationStringency.Silent
            )
    ...
    def test_from_lines_strict_raises_on_error(self):
        ...
        self.assertEqual(
            context.exception.tpe, MafValidationErrorType.HEADER_DUPLICATE_KEYS
        )
    ...
    def __test_from_lines_lenient_or_silent(self, validation_stringency):
        ...
        self.assertListEqual(
            list(header.keys()),
            [MafHeader.VersionKey, MafHeader.AnnotationSpecKey, "key1", "key2"],
        )
        self.assertEqual(
            [str(record.value) for record in header.values()],
        ...
        for record, clzz in zip(
            header.values(), [MafHeaderVersionRecord, MafHeaderRecord, MafHeaderRecord]
        ):
    ...
    def test_from_lines_ignores_errors(self):
        ...
        """
        self.__test_from_lines_lenient_or_silent(
    ...
    def test_dict_methods(self):
        ...
        self.assertEqual(
            header.scheme().annotation_spec(), expected_scheme.annotation_spec()
        )
        ...
        self.assertListEqual(list(header.keys()), [MafHeader.VersionKey, "key1"])
