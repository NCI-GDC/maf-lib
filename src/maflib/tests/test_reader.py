...
class TestMafReader(unittest.TestCase):
    ...
    Scheme = TestScheme()
    Version = "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.VersionKey, Scheme.version())
    AnnotationSpec = "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.AnnotationSpecKey, Scheme.annotation_spec())
    Names = Scheme.column_names()

    def test_different_scheme_in_header(self):
        version = "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.VersionKey, GdcV1_0_0_BasicScheme.version())
        ...
        self.assertListEqual([e.tpe for e in reader.validation_errors], [
    ...
    def test_different_column_names_but_same_named_scheme(self):
        lines = [TestMafReader.Version, TestMafReader.AnnotationSpec, "\t".join(reversed(TestMafReader.Names))]
    ...
    def test_use_scheme_from_header_basic(self):
        ...
        self.assertEqual(reader.header().scheme().version(), GdcV1_0_0_BasicScheme().version())

    def test_use_scheme_from_header_not_basic(self):
        version = "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.VersionKey, GdcV1_0_0_ProtectedScheme.version())
        annotation = "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.AnnotationSpecKey, GdcV1_0_0_ProtectedScheme.annotation_spec())
        lines = [version, annotation, "\t".join(GdcV1_0_0_ProtectedScheme().column_names())]

        reader = MafReader(lines=lines, scheme=None)
        self.assertIsNone(next(reader, None))
        self.assertEqual(len(reader.validation_errors), 0)
        self.assertEqual(reader.header().scheme().version(), GdcV1_0_0_ProtectedScheme().version())

    def test_use_default_scheme(self):
        lines = [TestMafReader.AnnotationSpec, "\t".join(GdcV1_0_0_BasicScheme().column_names())]
    ...
    def test_use_different_scheme(self):
        lines = [
            "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.VersionKey, GdcV1_0_0_ProtectedScheme.version()),
            "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.AnnotationSpecKey, GdcV1_0_0_ProtectedScheme.annotation_spec()),
        ...
        self.assertListEqual([e.tpe for e in reader.validation_errors],
                             [MafValidationErrorType.SCHEME_MISMATCHING_NUMBER_OF_COLUMN_NAMES])
        self.assertEqual(reader.header().scheme().version(), GdcV1_0_0_BasicScheme.version())
        self.assertEqual(reader.scheme().version(), GdcV1_0_0_ProtectedScheme().version())
        self.assertEqual(reader.scheme().annotation_spec(), GdcV1_0_0_ProtectedScheme.annotation_spec())
        self.assertEqual(reader.header().scheme().annotation_spec(), GdcV1_0_0_ProtectedScheme.annotation_spec())

    def test_iter(self):
        lines = [TestMafReader.Version,
    ...
    def test_reader_from_with_scheme(self):
        ...
        header = "%s%s %s" % (MafHeader.HeaderLineStartSymbol, MafHeader.VersionKey, scheme.version())
        ...
        reader = MafReader.reader_from(path=fn, validation_stringency=ValidationStringency.Silent, scheme=scheme)
